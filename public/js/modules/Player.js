 ...
 * Implements "lazy loading" for YouTube players.
 */
export default class Player {
  constructor(graphData, htmlOverlayContainer) {
  ...
  async play(node) {
    ...
    if (this.currentNode?.id !== node.id) {
        this.audio.pause();
        if (this.currentYtPlayer) this.currentYtPlayer.pauseVideo();
        this.currentYtPlayer = null;
    }
    ...
    if (node.sourceType === 'audio') {
        if (!node.audioUrl) {
          console.warn(`Audio URL missing for "${node.title}".`);
          this.stop();
          document.getElementById('songTitle').textContent = node.title;
          return;
        }
  ...
  togglePlay() {
    ...
    if (this.currentNode.sourceType === 'audio') {
        if (this.audio.paused) { this.audio.play(); playBtn.textContent = '⏸'; } 
        else { this.audio.pause(); playBtn.textContent = '▶'; }
    } else if (this.currentNode.sourceType === 'iframe' && this.currentYtPlayer) {
        const state = this.currentYtPlayer.getPlayerState();
        if (state === YT.PlayerState.PLAYING) this.currentYtPlayer.pauseVideo();
        else this.currentYtPlayer.playVideo();
    }
  }
  ...
  stop() {
    ...
    this.audio.src = '';
    if(this.currentYtPlayer) this.currentYtPlayer.stopVideo();
    this.currentYtPlayer = null;
    this.currentNode = null;
  ...
  createAndPlayYtPlayer(node) {
    ...
    return new Promise((resolve) => {
        ...
        wrapper.id = `html-overlay-${node.id}`;
  ...
  destroyYtPlayer(nodeId) {
      ...
      const wrapper = document.getElementById(`html-overlay-${nodeId}`);
  ...
  onPlayerStateChange(event, node) {
    ...
    const playBtn = document.getElementById('playBtn');
    switch(event.data) {
        case YT.PlayerState.ENDED: if (this.navigation) this.navigation.advance(); break;
        case YT.PlayerState.PLAYING: playBtn.textContent = '⏸'; break;
        case YT.PlayerState.PAUSED: playBtn.textContent = '▶'; break;
    }
  }

  setupEventListeners() {
    this.audio.addEventListener('timeupdate', () => this.updateProgress());
    this.audio.addEventListener('ended', () => { if (this.navigation) this.navigation.advance(); });
